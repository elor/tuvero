...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('MatrixModel', function (assert) {
    ...
    assert.equal(a.set(11, 5, 3), undefined, 'set() out of bounds returns undefined');
    assert.equal(a.set(4, 5, 3), a, 'set() inside bounds returns this');
    assert.equal(a.get(4, 5), 3, 'get() returns the set value');
    a.resize(6);
    assert.equal(a.get(4, 5), 3, 'get() returns the set value after resize (still in bounds)');
    a.resize(5);
    assert.equal(a.get(4, 5), undefined, 'get() returns the set value (now out of bounds)');
    ...
    assert.equal(a.get(0, 0), 0, 'get(0,0) after setting to 0 does not return undefined');
    ...
    assert.equal(a.get(1, 2), 0, 'get(1,2) after remove() now points to another element; returns 0');
    a = new MatrixModel(5);
