...
require([ "graph" ], function (Graph) {
  QUnit.test("Graph", function () {
    var g, res, a, b, c;

    g = new Graph();

    QUnit.equal(g.size(), 0, "empty graph size test");
    QUnit.equal(g.hasEdge(1, 2), false, "empty graph has no edges");
    QUnit.equal(g.removeVertex(2), undefined,
        "empty graph can't remove vertices");
    QUnit.equal(g.removeEdge(1, 2), undefined,
        "empty graph can't remove an edge");
    ...
    res = g.size();
    QUnit.equal(res, 2, "disconnected graph size test");
    res = g.isReachable();
    QUnit.equal(res, false, "disconnected graph reachability test");
    res = g.hasEdge(1, 2);
    QUnit.equal(res, false, "disconnected graph has no edge");
    res = g.getVertices();
    QUnit.deepEqual(res, [ 1, 2 ], "disconnected graph getVertices test");

    g.addEdge(2, 1);

    res = g.hasEdge(1, 2);
    QUnit.equal(res, true, "connected graph has an edge");
    res = g.isReachable();
    QUnit.equal(res, true, "connected graph is reachable");

    g.addEdge(2, 1);
    res = g.countEdges(2);
    QUnit.equal(res, 1, "countEdges after redundant addEdge");

    res = g.clone();
    QUnit.deepEqual(res, g, "graph clone");
    res = g.getConnectedSubgraphs();
    QUnit.deepEqual(res, [ g ], "connected subgraphs");
    ...
    res = g.isReachable();
    QUnit.equal(res, false, "larger disconnected graph is not reachable");

    a = (new Graph()).addVertex(1).addVertex(2).addEdge(1, 2);
    b = (new Graph()).addVertex(5).addVertex(6).addEdge(5, 6);
    res = g.getConnectedSubgraphs();
    ...
    res = g.countEdges(1);
    QUnit.equal(res, 1, "removeEdge Edge count 1");
    res = g.countEdges(2);
    QUnit.equal(res, 1, "removeEdge Edge count 2");
    res = g.countEdges(5);
    QUnit.equal(res, 0, "removeEdge Edge count 5");
    res = g.countEdges(6);
    QUnit.equal(res, 0, "removeEdge Edge count 6");

    res = g.getConnectedSubgraphs();
    QUnit.deepEqual(res, [ a, b, c ],
        "three connected subgraphs after removeEdge");

    g.removeVertex(5);
    a = (new Graph()).addVertex(1).addVertex(2).addVertex(6).addEdge(1, 2);

    res = g.size();
    QUnit.equal(res, 3, "removeVertex size test");
    // clone since the edge array contains explicit undefined
    res = g.clone();
    QUnit.deepEqual(res, a, "removeVertex clone deep test");

    g.addEdge(2, 6);
    g.removeVertex(1);
    a = (new Graph()).addVertex(2).addVertex(6).addEdge(6, 2);

    res = g.clone();
    QUnit.deepEqual(res, a, "removeVertex with edge clone deep test");
  });
});
