...
define(function () {
  return function (QUnit, getModule) {
    var FullMatrix, HalfMatrix, Matrix, Implements;
    ...
    QUnit.test("FullMatrix", function () {
      ...
      QUnit.equal(Implements(Matrix), '', 'Matrix interface validation');
      QUnit.equal(Implements(Matrix, FullMatrix), '',
          'FullMatrix interface match');
    ...
    QUnit
        .test(
            ...
            function () {
              ...
              QUnit.deepEqual(Matrix.mult(a, b, out), expected,
              ...
              QUnit.deepEqual(Matrix.multVec(a, vec), [],
                  "multVec with empty matrix");
              QUnit.deepEqual(Matrix.vecMult(vec, a), [],
                  "vecMult with empty matrix");
              QUnit.deepEqual(Matrix.multVec(b, vec), [ 0, 40, 6 ],
                  "multVec check");
              QUnit.deepEqual(Matrix.vecMult(vec, b), [ 14, 27, 10 ],
                  "vecMult check");
              ...
              QUnit.deepEqual(Matrix.getRow(b, 0), [ 0, 0, 0 ],
                  "getRow with empty row");
              ...
              QUnit.deepEqual(Matrix.rowSums(a), [],
                  "rowSums with matrix of size 0");
              QUnit.deepEqual(Matrix.rowSums(b), [ 0, 21, 3 ],
                  "rowSums with sparse matrix");
              QUnit.deepEqual(Matrix.colSums(a), [],
                  "colSums with matrix of size 0");
