...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('BinningReferenceListModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(BinningReferenceListModel, ListModel), 'BinningReferenceListModel is subclass of ListModel');
    success = false;
    try {
    ...
    list.insert(0, 24);
    assert.deepEqual(bin.asArray(), [24, 4, 14], 'order of original list is preserved');
    list.push(1);
    list.push(12);
    list.push(53);
    assert.equal(binlist.length, 4, 'More insertions: 4 bins');
    list.pop();
    assert.equal(binlist.length, 3, 'only 3 bins after the removal of a unique number');
    list = new ListModel([4, 1, 2, 53, 16, 5, 8, 9, 0, 7]);
    binlist = new BinningReferenceListModel(list, binningFunction);
    assert.equal(binlist.length, 10, 'auto-initialization from existing values');
    assert.equal(binlist.getBin(3).get(0), 53, 'binlist contains actual values, not indices');
  });
});