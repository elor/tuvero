...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('CombinedReferenceListModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(CombinedReferenceListModel, ListModel), 'CombinedReferenceListModel is subclass of ListModel');
    success = false;
    try {
    ...
    assert.deepEqual(combined.asArray(), [1, 3, 5, 2, 4, 6], 'multiple lists are appended');
    list2.insert(1, 13);
    assert.deepEqual(combined.asArray(), [1, 3, 5, 2, 13, 4, 6], 'list offsets are correct');
    list1.clear();
    assert.deepEqual(combined.asArray(), [2, 13, 4, 6], 'clear() shifts all following elements towards the front');
    list1 = new ListModel([1, 3, 5]);
    combined = new CombinedReferenceListModel(list1, new CombinedReferenceListModel(list1), new CombinedReferenceListModel(list1));
    assert.deepEqual(combined.asArray(), [1, 3, 5, 1, 3, 5, 1, 3, 5], 'combining the same list three times');
    list1.push(7);
    assert.deepEqual(combined.asArray(), [1, 3, 5, 7, 1, 3, 5, 7, 1, 3, 5, 7], 'pushing to the triply-combined list');
  });
});