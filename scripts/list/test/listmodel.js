...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('ListModel', function (assert) {
    ...
    assert.equal(listener.insertions, 5, 'number of fired insert events matches number of insertions');
    ...
    assert.equal(list.get(5), undefined, 'get slightly out of bounds (index 5)');
    assert.equal(list.get(1234567890), undefined, 'get wildly out of bounds (index 1234567890)');
    assert.equal(list.indexOf(1), 0, 'indexOf: first element');
    assert.equal(list.indexOf(5), 4, 'indexOf: last element');
    assert.equal(list.indexOf('unavailable'), -1, 'indexOf: unavailable element');
    ...
    assert.equal(list.remove(123), undefined, 'remove out of bounds does nothing');
    ...
    assert.deepEqual(list.asArray(), [2, 123], 'list.set actually sets the value');
    assert.equal(listener.length, 2, 'resize event balanced on set() (may have been fired twice)');
    listener.reset();
    assert.equal(list.set(123, 321), undefined, "set out of bounds doesn't to anything");
    assert.deepEqual(list.asArray(), [2, 123], "set out of bounds really doesn't to anything");
    assert.equal(listener.insertions, 0, "set out of bounds doesn't fire insert event");
    assert.equal(listener.removals, 0, "set out of bounds doesn't fire remove event");
    list.clear();
    ...
    ret = list.map(function (num, index, thelist) {
      ...
      assert.equal(num, list.length - index, 'first argument is the list content' + index);
      assert.equal(thelist, list, 'map(): third function argument is the list, ' + index);
      i += 1;
      return num * num;
    }, 5);
    res = [16, 9, 4, 1];
    ...
    assert.deepEqual(list.asArray(), [], 'asArray returns empty array after clear');
    obj = {
      tmp: true,
      tmpLong: 'very much so'
    };
    list.push(obj);
    assert.equal(list.get(0), obj, 'objects are directly referenced, not copied');
    assert.equal(list.pop(), obj, 'pop returns the popped object directly');
    assert.equal(list.length, 0, 'popped size is 0');
    assert.deepEqual(list.asArray(), [], 'asArray returns empty array after pop');
    list = new ListModel();
    list.push(3);
    list.push(2);
    list.push(3);
    ...
    assert.deepEqual(data, [5, 3, 4, 2, 3, 1], 'save() uses a list representation for raw types');
    list = new ListModel();
    assert.ok(list.restore(data), 'restore() returns');
    assert.deepEqual(list.asArray(), [5, 3, 4, 2, 3, 1], 'restore() restores the whole list');
    list.clear();
    list.push('Tuvero');
    list.push('is');
    list.push('awesome');
    data = list.save();
    assert.deepEqual(data, ['Tuvero', 'is', 'awesome'], 'saved data object is just an array of strings');
    list.clear();
    ...
    list = new ListModel();
    assert.ok(list.restore(data, DummyModel), 'restore() with Model Constructor returns');
    ...
    assert.ok(list.restore(data, DummyModel), 'restore() with Model Factory returns');
    assert.equal(list.length, data.length, 'restore() restores the length');
    assert.equal(list.get(0).data, 'asd5', 'restore() constructs the object');
    assert.equal(list.get(1).data, 'asd3', 'restore() constructs the object');
    assert.equal(list.get(2).data, 'asd4', 'restore() constructs the object');
