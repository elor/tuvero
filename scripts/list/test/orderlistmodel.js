...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('OrderListModel', function (assert) {
    var list, ref;
    assert.ok(extend.isSubclass(OrderListModel, ListModel), 'OrderListModel is subclass of ListModel');
    ...
    assert.deepEqual(list.asArray(), ref, 'elements get ordered properly');
    ref = [6, 3, 1, 5, 7, 2, 0, 4];
    list.enforceOrder(ref);
