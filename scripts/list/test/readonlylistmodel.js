...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('ReadonlyListModel', function (assert) {
    ...
    assert.equal(rolist.get(5), undefined, 'get slightly out of bounds (index 5)');
    assert.equal(rolist.get(1234567890), undefined, 'get wildly out of bounds (index 1234567890)');
    assert.equal(rolist.indexOf(1), 0, 'indexOf: first element');
    assert.equal(rolist.indexOf(5), 4, 'indexOf: last element');
    assert.equal(rolist.indexOf('unavailable'), -1, 'indexOf: unavailable element');
    ...
    assert.deepEqual(rolist.asArray(), [2, 123], 'list.set actually sets the value');
    ...
    assert.deepEqual(rolist.asArray(), [2, 123], "set out of bounds really doesn't to anything");
    ...
    ret = rolist.map(function (num, index, thelist) {
      ...
      assert.equal(num, rolist.length - index, 'first argument is the list content' + index);
      assert.equal(thelist, rolist, 'map(): third function argument is the list, ' + index);
      i += 1;
      return num * num;
    }, 5);
    res = [16, 9, 4, 1];
    ...
    assert.deepEqual(rolist.asArray(), [], 'asArray returns empty array after clear');
    obj = {
      tmp: true,
      tmpLong: 'very much so'
    };
    list.push(obj);
    assert.equal(rolist.get(0), obj, 'objects are directly referenced, not copied');
    assert.equal(list.pop(), obj, 'pop returns the popped object directly');
    assert.equal(rolist.length, 0, 'popped size is 0');
    assert.deepEqual(rolist.asArray(), [], 'asArray returns empty array after pop');
  });
});