...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('ReferenceListModel', function (assert) {
    ...
    assert.equal(refs.length, matches.length, 'number of teams match after initialization');
    ...
    assert.equal(listener.numEvents, 2, 'both "finish" events propagate to the matchref');
