      ...
      History = {
        ...
        addCorrection: function(tournamentid, before, after) {
          ...
          for (game in tournament.games) {
            ...
            if (before[0] == game[0] && before[1] == game[1]
                && before[2] == game[2] && before[3] == game[3]
                && (!before[4] || before[4] == game[4])
                && (!before[5] || before[5] == game[5])) {
              game[0] = after[0];
              game[1] = after[1];
              game[2] = after[2];
              game[3] = after[3];
          ...
          for (tournamentid = 0; tournamentid < History.numTournaments(); tournamentid += 1) {
            ...
            for (roundid = 0; roundid < numrounds; roundid += 1) {
              ...
              if (games && games.length) {
                lines.push('#' + Tournaments.getName(tournamentid) + ' Runde '
                    + (roundid + 1) + ' Spiele,');

                lines
                    .push('No1,No2,Name1_1,Name1_2,Name1_3,Name2_1,Name2_2,Name2_3,Points1,Points2');
                ...
                if (vote[2] === roundid) {
                  if (firstvote) {
                    ...
                    lines.push('#' + Tournaments.getName(tournamentid)
                        + ' Runde ' + (roundid + 1) + ' Lose,');
                    lines.push('No,Name1,Name2,Name3,Typ');
                  }

                  names = Team.getNames(vote[1]);
                  line = [];
