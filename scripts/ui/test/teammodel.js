...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('TeamModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(TeamModel, IndexedModel), 'TeamModel is subclass of IndexedModel');
    listener = {
      updatecount: 0,
      /**
       * Callback listener
    ...
    team = new TeamModel();
    assert.equal(team.length, 1, 'empty initialization generates an empty team player');
    assert.equal(team.getPlayer(0).getName(), PlayerModel.NONAME, 'empty initialization generates an default team player name');
    assert.equal(team.getID(), -1, 'empty initialization sets id to -1');
    assert.equal(team.getPlayer(-1), undefined, 'getPlayer(-1) returns undefined');
    assert.equal(team.getPlayer(5), undefined, 'out of bounds getPlayer returns undefined');
    ...
    assert.equal(team.length, players.length, 'proper initialization: team length');
    ...
    assert.equal(listener.updatecount, 3, 'player name updates propagate through to TeamModel');
  });
});