...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('CorrectionReferenceModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(CorrectionReferenceModel, CorrectionModel), 'CorrectionReferenceModel is subclass of CorrectionModel');
    teams = new ListModel();
    teams.push(5);
    teams.push(3);
    teams.push(6);
    ...
    assert.equal(reference.before.result, result, 'before result reference is set');
    assert.equal(reference.after.result, result2, 'after result reference is set');
    assert.deepEqual(reference.before.teams, [0, 1], 'before teams correctly referenced');
    assert.deepEqual(reference.after.teams, [6, 13], 'after teams correctly referenced');
    assert.deepEqual(reference.before.score, result.score, 'before score is correct');
    assert.deepEqual(reference.after.score, result2.score, 'after score is correct');
