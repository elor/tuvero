...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('Type', function (assert) {
    ...
    ref.forEach(function (typename, index) {
      assert.equal(Type(types[index]), typename, 'Type ' + typename + ' detected');
    });
    ...
    functionnames.forEach(function (functionname) {
      ...
      assert.equal(Type(Type[functionname]), 'function', 'Type.' + functionname + '() is a function');
    });
    ...
    types.forEach(function (type, typeindex) {
      functionnames.forEach(function (functionname, functionindex) {
        ...
        assert.equal(Type[functionname](type), expected, 'Type.' + functionname + ' on a ' + Type(type) + ' is ' + expected);
      });
    });
    ...
    types.forEach(function (type, typeindex) {
      ref.forEach(function (refname, functionindex) {
        var expected = functionindex === typeindex;
        assert.equal(Type.is(type, refname), expected, 'Type.is(obj, "' + refname + '") on a ' + Type(type) + ' is ' + expected);
      });
    });
    types.forEach(function (type, typeindex) {
      constructors.forEach(function (constructor, functionindex) {
        var expected = functionindex === typeindex;
        assert.equal(Type.is(type, constructor), expected, 'Type.is(obj, ' + constructor + ') on a ' + Type(type) + ' is ' + expected);
      });
    });
  });
});