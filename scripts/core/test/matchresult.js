...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('MatchResult', function (assert) {
    ...
    assert.deepEqual(result.getGroup(), -1, 'empty construction -> group == -1');
    score = [13, 7];
    try {
    ...
    assert.equal(result.match, undefined, 'result.match got kicked out of the API');
    ...
    assert.ok(success, 'construction with a team array does not work anymore (API change)');
     ...
     * isRunningMatch()
     */
    assert.equal(result.isRunningMatch(), false, 'results are not running matches');
  });
});