...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('Listener', function (assert) {
    ...
    assert.equal(listener.numEmitters(), 1, 'listener initializes with an emitter');
    emitter.emit('update');
    assert.equal(listener.updatecount, 1, 'update event received from emitter1');
    ...
    assert.equal(listener.numEmitters(), 1, 'emitter.destroy() unregisters the listener');
    emitter.registerListener(listener);
    assert.equal(listener.numEmitters(), 2, 're-registering the emitter');
    emitter.destroy();
    assert.equal(listener.numEmitters(), 1, 'emitter.destroy() unregisters the listener');
    listener.destroy();
    assert.equal(listener.numEmitters(), 0, 'listener.destroy() unregisters all emitters');
    assert.equal(emitter2.numListeners(), 0, 'listener.destroy() unregisters the listener from all emitters');
    ...
    assert.equal(ref, 124, 'bind-listening for multiple event types ' + 'with data object');
    emitter.destroy();
    emitter = new Emitter();
    ref = undefined;
    ...
    emitter2.registerListener(listener);
    listener.destroy();
    assert.equal(emitter.listeners.length, 0, 'memleak: first emitter was unregistered');
    assert.equal(emitter2.listeners.length, 0, 'memleak: second emitter was unregistered');
    ...
    listener = new Listener(emitter);
    emitter.registerListener(listener);
    assert.equal(listener.emitters.length, 1, 'double registration of a listener is prevented');
    listener.resetcount = 0;
    listener.onreset = function () {
      this.resetcount += 1;
    };
    emitter.emit('reset');
    assert.equal(listener.resetcount, 1, 'double invocation of a listener is prevented');
  });
});