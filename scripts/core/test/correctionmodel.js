...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('CorrectionModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(CorrectionModel, Model), 'CorrectionModel is subclass of Model');
    ...
    assert.deepEqual(correction.before, new MatchResult(), 'default construction of pre-correction result');
    assert.deepEqual(correction.after, new MatchResult(), 'default construction of post-correction result');
    ...
    assert.deepEqual(correction.before, result, 'restore() restores the pre-correction result');
    assert.deepEqual(correction.after, result2, 'restore() restores the post-correction result');
  });
});