...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('MatchModel', function (assert) {
    ...
    assert.equal(game.finish([]), undefined, 'game.finish([]) fails');
    assert.equal(game.finish([3, 2, 1]), undefined, 'game.finish([3,2,1]) fails');
    ...
    assert.equal(game.isRunningMatch(), false, 'isRunningMatch() of a match with duplicate teams');
    game = new MatchModel([0, undefined], 0, 0);
    assert.equal(game.isRunningMatch(), false, 'isRunningMatch() of a match with an undefined team');
    ...
    game = new MatchModel();
    assert.equal(game.restore(data), true, 'restore() with undefined team works');
    assert.equal(game.getTeamID(1), undefined, 'second team still is undefined');
  });
});