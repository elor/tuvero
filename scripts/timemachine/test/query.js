...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('Query', function (assert) {
    ...
    assert.deepEqual(query.filter(), [], 'ALL-query on cleared localStorage returns no results');
    ...
    assert.deepEqual(new Query(undefined).filter(), [], 'ALL-query on post-test cleared localStorage returns no results');
  });
});