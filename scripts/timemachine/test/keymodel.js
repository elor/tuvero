...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('KeyModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(KeyModel, Model), 'KeyModel is subclass of Model');
    ...
    assert.equal(KeyModel.isValidKey(key.toString()), true, 'serialized init-key is a valid key');
    assert.equal(key.isRoot(), true, 'init-key is an init key');
    assert.equal(key.isRelated(key), true, 'init-key is actually related to itself');
    assert.equal(key.isRelated(key.toString()), true, 'key string is related to itself');
    assert.equal(key.isEqual(key), true, 'key is equal to itself');
    assert.equal(key.isEqual(key.toString()), true, 'key string is equal to itself');
    date = new Date(key.startDate);
    assert.ok(date, 'startDate can be converted to an instance of Date');
    assert.equal(date.toISOString(), key.startDate, 'Date conversion is fully reversible');
    ...
    assert.equal(key2.target, Presets.target, 'save-key target matches current target');
    assert.notEqual(key2.startDate, key2.saveDate, 'save-key dates are not equal');
    assert.equal(key2.startDate, key.startDate, 'both keys have the same start date');
    date = new Date(key.saveDate);
    assert.ok(date, 'saveDate can be converted to an instance of Date');
    assert.equal(date.toISOString(), key.saveDate, 'Date conversion is fully reversible');
    assert.equal(KeyModel.isValidKey(key2), true, 'key2 is a valid key');
    assert.equal(key2.isRoot(), false, 'save-key is no init key');
    ...
    assert.equal(KeyModel.isValidKey(ref), true, 'ref is a valid key: ' + ref);
    ...
    assert.equal(key.isEqual(ref), true, 'key is equal to its construction string');
    assert.equal(KeyModel.isValidKey(key), true, 'string-constructed key is a valid key');
    assert.equal(key.isRoot(), false, 'string-key is no init key');
    ...
    assert.equal(KeyModel.isTuveroKey(ref), true, 'cross-target key is a tuvero key: ' + ref);
    assert.equal(KeyModel.isValidKey(ref), false, 'cross-target key is not valid: ' + ref);
    ...
    function testkey(keystring, keydescription) {
      ...
      assert.equal(success, true, 'forbidden key is intercepted: ' + keydescription);
    }
