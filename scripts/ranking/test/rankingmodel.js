...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('RankingModel', function (assert) {
    ...
    assert.equal(Options.byepointswon, 13, 'Options.byepointswon is set properly');
    assert.equal(Options.byepointslost, 7, 'Options.byepointslost is set properly');
    ...
    assert.ok(extend.isSubclass(RankingModel, Model), 'RankingModel is a subclass of Model and, hence, Emitter');
    ...
    assert.deepEqual(ranking.get(), ref, 'empty initialization produces a valid ranking');
    ...
    assert.deepEqual(ranking.extDeps, ['wins', 'points'], 'empty ranking does not ignore extra dependencies');
    ...
    assert.equal(ranking.dataListeners.numgames.isPrimary(), true, 'numgames is primary');
    assert.equal(ranking.dataListeners.wins.isPrimary(), true, 'wins is primary');
    assert.equal(ranking.dataListeners.saldo.isPrimary(), false, 'saldo is secondary');
    assert.equal(ranking.dataListeners.points.isPrimary(), true, 'points is primary');
    assert.equal(ranking.dataListeners.lostpoints.isPrimary(), true, 'lostpoints is primary');
    ranking = new RankingModel(['numgames', 'wins'], 5, ['saldo']);
    assert.equal(ranking.dataListeners.saldo.isPrimary(), false, 'extraDependency: saldo is secondary');
    assert.equal(ranking.dataListeners.points.isPrimary(), true, 'extraDependency: points is primary');
    assert.equal(ranking.dataListeners.lostpoints.isPrimary(), true, 'extraDependency: lostpoints is primary');
    ranking = new RankingModel(['numgames', 'wins', 'saldo', 'points'], 5);
    listener.reset();
    ...
    assert.equal(ranking.dataListeners.buchholz.isPrimary(), false, 'buchholz is secondary');
    assert.equal(ranking.dataListeners.finebuchholz.isPrimary(), false, 'finebuchholz is secondary');
    assert.equal(ranking.dataListeners.gamematrix.isPrimary(), false, 'gamematrix is secondary');
    assert.equal(ranking.dataListeners.points.isPrimary(), true, 'points is primary');
    assert.equal(ranking.dataListeners.wins.isPrimary(), true, 'wins is primary');
    assert.equal(ranking.dataListeners.winsmatrix.isPrimary(), true, 'winsmatrix is primary');
    ...
    rankingobject = ranking.get();
    assert.deepEqual(rankingobject, ref, 'bye() is applied properly to all basic components');
    ranking.bye([0, 1, 3]);
    ref = {
      components: ['numgames', 'wins', 'points', 'lostpoints', 'saldo', 'buchholz', 'finebuchholz'],
      ids: [0, 1, 2, 3, 4],
    ...
    rankingobject = ranking.get();
    assert.deepEqual(rankingobject, ref, 'bye() with multiple teams works (all teams receive bye)');
    ...
    rankingobject = ranking.get();
    assert.deepEqual(rankingobject, ref, 'buchholz and finebuchholz include bye-induced wins');
    ...
    rankingobject = ranking.get();
    assert.deepEqual(rankingobject, ref, 'resize() is able to append a team to the ranking');
    ranking.resize(3);
    ref = {
      components: ['numgames', 'wins', 'points', 'lostpoints', 'saldo', 'buchholz', 'finebuchholz'],
      ids: [0, 1, 2],
    ...
    rankingobject = ranking.get();
    assert.deepEqual(rankingobject, ref, 'resize() shrinks without changing the actual values');
    ...
    rankingobject = ranking.get();
    assert.deepEqual(rankingobject, ref, 'reset() restores to an empty ranking');
    ...
    assert.equal(ranking.length, 5, 'restore() of an id-only ranking also restores the length');
    ref = {
      components: ['id'],
      ids: [0, 1, 2, 3, 4],
      ranks: [0, 1, 2, 3, 4],
