...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('RankingComponentIndex', function (assert) {
    ...
    assert.deepEqual(chain.dependencies, ['points', 'wins', 'numgames'], 'multiple dependencies, correct order.');
    chainlength = 0;
    while (chain) {
      chainlength += 1;
      chain = chain.nextcomponent;
    }
    assert.equal(chainlength, components.length + 1, 'chain has correct length (components.length + DUMMYCOMPONENT)');
