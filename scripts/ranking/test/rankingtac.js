...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('TAC Ranking', function (assert) {
    ...
    assert.equal(ranking.dataListeners.tac.isPrimary(), true, 'tac is a primary dataListener');
    ref = {
    ...
    ret = ranking.get();
    assert.deepEqual(ret, ref, 'undo a correction yields original result' + ' ("one point for 0:8" is undone correctly)');
