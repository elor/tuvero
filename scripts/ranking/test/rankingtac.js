...
test('TAC Ranking', () => {
  ...
  expect(ranking.dataListeners.tac.isPrimary(), 'tac is a primary dataListener').toBe(true)
  ref = {
  ...
  ret = ranking.get()
  expect(ret, 'empty ranking: correct TAC score').toEqual(ref)
  ...
  expect(ret, 'second ranking is correct (0 -> 1)').toEqual(ref)
  ...
  ret = ranking.get()
  expect(ret, 'aborted game points are saldo only').toEqual(ref)
  ...
  ret = ranking.get()
  expect(ret, 'aborted game points, equal').toEqual(ref)
  ...
  ret = ranking.get()
  expect(ret, 'correct() works correctly with TAC').toEqual(ref)
  ...
  expect(
    ret,
    'undo a correction yields original result' + ' ("one point for 0:8" is undone correctly)'
  ).toEqual(ref)
  ...
  ret = ranking.get()
  expect(ret, 'tac accepts byes').toEqual(ref)
  ranking.bye(1)
  ref.tac[1] *= 2
  ret = ranking.get()
  expect(ret, 'tac accepts multiple byes').toEqual(ref)
