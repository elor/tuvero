...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('RankingMapper', function (assert) {
    ...
    assert.ok(extend.isSubclass(RankingMapper, Model), 'RankingMapper is subclass of Model');
    teams = new ListModel();
    teams.push(5);
    teams.push(4);
    teams.push(3);
    ...
    assert.deepEqual(ranking.get(), ref, 'only the ids get re-mapped to external ids, not the displayOrder');
    ...
    listener.onupdate = function (emitter) {
      ...
      assert.equal(emitter, ranking, 'callback: emitter is ranking (safety check)');
      assert.deepEqual(emitter.get(), reference, 'ids remapped after second result, inside callback');
    };
    ...
    assert.deepEqual(ranking.get(), ref, 'ids remapped after second result, outside of callback');
  });
});