...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('TournamentListModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(TournamentListModel, IndexedListModel), 'TournamentListModel is subclass of IndexedListModel');
    ...
    assert.deepEqual(list.tournamentIDsForEachTeam(), [], 'no tournament ids yet');
    ...
    assert.deepEqual(list.tournamentIDsForEachTeam(), ref, 'tournament ids for single tournament are correct');
    tournament = TournamentIndex.createTournament('round', ['sonneborn', 'id']);
    tournament.addTeam(1);
    ...
    list.push(tournament);
    ref = [0, 1, 0, 1, 0, 1];
    assert.deepEqual(list.tournamentIDsForEachTeam(), ref, 'tournament ids for two tournaments are correct');

    // HACK! DO NOT ACCESS DIRECTLY!
    list.get(0).state.forceState('finished');
    ref = [undefined, 1, undefined, 1, undefined, 1];
    assert.deepEqual(list.tournamentIDsForEachTeam(), ref, 'ids of finished tournaments are ignored (undefined)');
    savedata = list.save();
    assert.ok(savedata, 'save() returns properly');
    list = new TournamentListModel();
    assert.ok(list.restore(savedata), 'restore() returns true');
    assert.deepEqual(list.tournamentIDsForEachTeam(), ref, 'restore() restores the ids for all players');
