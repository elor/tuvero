...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('TournamentModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(TournamentModel, PropertyModel), 'TournamentModel is subclass of PropertyModel');
    ...
    assert.equal(state.get(), 'initial', 'tournament is constructed in initial state');
    ...
    assert.equal(tournament.getVotes('unknownVoteType'), undefined, 'undefined vote type returns undefined');

    /*************************************************************************
     * run
     ************************************************************************/

    assert.equal(tournament.run(), undefined, 'run() aborts with insufficient teams');
    ...
    assert.ok(match.finish([12345, -53]), 'match.finish() with invalid score points');
    ...
    assert.equal(combinedHistory.length, 1, 'combinedHistory contains an element');
    assert.equal(tournament.finish(), false, 'tournament cannot be finished when there are open matches');
    ...
    assert.equal(state.get(), 'idle', 'auto-transition to idle state after last match');
    assert.equal(byes.length, 0, 'votes are cleared before transition to idle state');
    ref = {
    ...
    assert.equal(combinedHistory.length, 2, 'combined history contains a current match, ' + 'a finished match and the current bye');
    match = matches.get(0);
    ...
    assert.equal(tournament.run(), undefined, 'tournament cannot be un-finished');
    assert.deepEqual(ranking.get(), ref, 'ranking validation');
    tournament = new TournamentModel(['wins', 'saldo']);
    ...
    assert.deepEqual(teams.asArray(), ref.getTeams().asArray(), 'restore() restored the teams');
    assert.equal(state.get(), 'running', 'restore() restored the "running" state');
    ...
    assert.deepEqual(ranking.get(), ref.getRanking().get(), 'restore() restored the whole ranking');
    result = history.get(0);
    assert.equal(history.length, 1, 'restore(): history size');
    assert.equal(result.length, 2, 'restore(): restored number of teams in history result');
