...
export default (function (QUnit, getModule) {
  ...
  QUnit.test('RoundTournamentModel', function (assert) {
    ...
    assert.ok(extend.isSubclass(RoundTournamentModel, TournamentModel), 'RoundTournamentModel is subclass of TournamentModel');
    ...
    assert.ok(!tournament.run(), 'too few teams prohibit running the tournament');
    ...
    assert.ok(matches.get(0).getID() < matches.get(1).getID(), 'match ids are sorted');
    ...
    assert.equal(matches.length, 0, 'second round does not start automatically');
    ...
    assert.deepEqual(tournament.getVotes('bye').get(0), 4, 'second round: correct bye');
    ...
    assert.deepEqual(tournament.getVotes('bye').get(0), 3, 'third round: correct bye');
    matches.get(0).finish([11, 13]);
    matches.get(0).finish([10, 13]);
    assert.equal(state.get(), 'idle', 'idle after games finished');
    assert.equal(matches.length, 0, 'fourth round does not start automatically');
    ...
    assert.deepEqual(tournament.getVotes('bye').get(0), 2, 'fourth round: correct bye');
    ...
    assert.deepEqual(tournament.getVotes('bye').get(0), 1, 'fifth round: correct bye');
    matches.get(0).finish([13, 5]);
    matches.get(0).finish([13, 7]);
    assert.equal(state.get(), 'finished', '5-team tournament finished after 5 rounds');
    ...
    matches.get(0).finish([13, 4]);
    matches.get(0).finish([13, 9]);
    assert.equal(state.get(), 'finished', '4-team tournament is finished after 3 rounds');
